...
struct GameState {
    ...
    250, 370,                  // paddle1
    250, 20,                   // paddle2
    {}, false, false, 0        // briques, game_over, you_win, score
};
...
void ball_physics_thread(void *arg) {
    ...
    while (true) {
        ...
        if (frame_counter > 3) {
            ...
            if (g_game_state.ball_x <= 10) g_game_state.ball_dx = 1;
            if (g_game_state.ball_x >= 630) g_game_state.ball_dx = -1;
            ...
            int all_destroyed = 0;
            for (int i = 0; i < 12; i++)
                if (g_game_state.bricks[i].destroyed) all_destroyed++;
            if (all_destroyed == 12)
