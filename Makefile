...
CPP_Linux=g++
LD_Linux=ld
COMPOP_Linux=-fno-stack-protector -m32 -O3 $(EXTRA_FLAGS)
...
Ball Paddle Brick BreakoutGame InputManager renderer_ascii
...
run: $(KERNEL_OBJ)
	$(QEMU) -display curses -serial stdio -net nic,model=ne2k_isa -net user,tftp=./build/boot -cdrom ./build/boot/grub.iso

run_gui: $(KERNEL_OBJ)
	$(QEMU) -nographic -serial mon:stdio -kernel ./build/boot/sextant.elf
